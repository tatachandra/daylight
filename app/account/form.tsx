...
export default function AccountForm({googleEnabled=false,signedInEmail,googleLinked=false}:{googleEnabled?:boolean;signedInEmail?:string;googleLinked?:boolean}){
 ...
 <button className="signin-button" disabled={busy} type="submit">{busy?'Please wait…':({signin:'Sign in',signup:'Create account',forgot:'Send password reset link',reset:'Save new password',verify:'Resend verification email'})[mode]}</button></form>;
 return <><h1>{({signin:'Welcome back',signup:'Create your account',forgot:'Reset your password',reset:'Choose a new password',verify:'Verify your email'})[mode]}</h1><p>{mode==='forgot'?'Enter your account email. We’ll send a link to choose a new password.':mode==='verify'?'Enter your account email to request another verification email. Check your spam folder too.':mode==='reset'?'Choose a new password of at least 15 characters. You’ll sign in again afterward.':'Your meal and workout logs stay private to your account.'}</p>{googleEnabled&&(mode==='signin'||mode==='signup')?<><button type="button" style={googleButton} disabled={busy} onClick={()=>void signInWithGoogle()}>{busy?'Opening Google…':'Continue with Google'}</button><p style={{fontSize:14}}>Use your Google account. No separate username or password needed.</p><details key={mode} style={{marginTop:24}}><summary style={{cursor:'pointer',marginBottom:16}}>Use email instead</summary>{emailForm}</details></>:emailForm}
 {message&&<p role="status" style={{padding:'12px 0'}}>{message}</p>}
 <nav aria-label="Account options" style={{display:'flex',gap:12,flexWrap:'wrap',marginTop:24}}>{mode!=='signin'&&<button disabled={busy} onClick={()=>switchMode('signin')}>Sign in</button>}{mode!=='signup'&&<button disabled={busy} onClick={()=>switchMode('signup')}>Create account</button>}{mode==='signin'&&<><button disabled={busy} onClick={()=>switchMode('forgot')}>Forgot password?</button><button disabled={busy} onClick={()=>switchMode('verify')}>Resend verification email</button></>}</nav></>;
}
