...
export async function GET(request:Request){
 const user=await getChatGPTUser();if(!user)return response({error:'Sign in to access your private logs.'},401);
 ...
 ]);const entries:LogEntry[]=entryRows.results.map(row=>({id:row.id,day:row.day,data:JSON.parse(row.data_json),createdAt:row.created_at}));const settings:WellnessSettings=settingRow?JSON.parse(settingRow.data_json):DEFAULT_SETTINGS;
...
export async function POST(request:Request){
 const user=await getChatGPTUser();if(!user)return response({error:'Sign in before saving your private logs.'},401);
 ...
 try{const db=wellnessDb();const now=new Date().toISOString();const action=input.data;
  ...
  if(action.action==='save-settings'){await db.prepare('INSERT INTO wellness_settings(user_id,data_json,updated_at) VALUES(?,?,?) ON CONFLICT(user_id) DO UPDATE SET data_json=excluded.data_json,updated_at=excluded.updated_at').bind(user.userId,JSON.stringify(action.settings),now).run();return response({saved:true});}
